...
In state estimation, the challenge is estimating the states of drone with only onboard sensors and processor. Especially visual odometry with camera(mono or stereo) will be the biggest one. Because without outer sensors, it has huge errors on estimating the state of the drone includes position, attitude, etc. Even though the processors of drones have been advanced, it is not enough to analyze and use in high rate, real time. Accurate but slow vision and inaccurate but fast IMU, we have to get over it by using both of them properly.
...
For now, we are considering which algorithm we will use, SVO(Semi-Direct Vosual Odometry) from UZH seems most proper one. According to their papers, it is much accurate that others(i.e. ORB, LSD), and fast enough to publish data with 20Hz(Intel Core i7-2760QM CPU). With NVIDIA Jetson Xavier, we expect similar or better result.
